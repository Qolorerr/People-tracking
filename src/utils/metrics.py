...
class AdaptiveMetrics:
    def __init__(self, window_size: int = 30):
        ...
        self.window: collections.deque[dict[str, int | float]] = collections.deque(
            maxlen=window_size
        )
    ...
    def update(
        ...
        self.current["match_ratio"] = (
            len(matches) / len(frame_detections) if len(frame_detections) > 0 else 1.0
        )
        self.current["prediction_error"] = 1.0 - self._avg_iou(
            matches, frame_detections, track_predictions
        )
    ...
    @staticmethod
    def _avg_iou(
        matches: list[tuple[int, int]], frame_detections: Tensor, track_predictions: Tensor
    ) -> float:
        if matches:
    ...
    def _feature_consistency(self, frame_features: dict[int, Tensor]) -> float:
        ...
        self.previous_features = {
            tid: feat.clone().detach() for tid, feat in frame_features.items()
        }
