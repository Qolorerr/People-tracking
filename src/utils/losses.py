...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(self, inputs: Tensor, targets: LongTensor):
        """
        Args:
        ...
        zeros = torch.zeros(log_probs.size(), device=inputs.device, dtype=torch.long)
        targets = zeros.scatter_(1, targets.unsqueeze(1).data, 1)
